...
AddEventHandler('esx_status:loaded', function(status)
	...
	TriggerEvent('esx_status:registerStatus', 'drug', 0, '#CF380F', function(status)
		return Config.Visible
	end, function(status)
...
local drugWait, aiming, holding, melee, still, stealth, jacking = 0, 0, 0, 0, 0, 0, 0
...
AddEventHandler('esx_status:onTick', function(data)
	...
	for k, v in pairs(data) do
		...
		elseif v.name == 'drug' then drug = v.percent
			local drug = 0
			local calm = true
			...
			if drugWait > 0 then drugWait = drugWait - 1
			...
			if drug ~= 0 then
				if drug > 0 then TriggerEvent('esx_status:add', 'drug', drug) else TriggerEvent('esx_status:remove', 'drug', -drug) end
			end
		end
	end
